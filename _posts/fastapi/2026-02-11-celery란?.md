...
태스크 결과는 `AsyncResult`를 통해 조회합니다.
...
`--concurrency=4`는 동시에 처리할 수 있는 태스크 수를 의미합니다. 기본은 **멀티 프로세스(prefork)** 방식입니다.
...
실무에서는 `soft_time_limit`으로 먼저 경고받아 정리할 시간을 확보하고, `time_limit`으로 최종 강제 종료하는 패턴을 사용합니다.
